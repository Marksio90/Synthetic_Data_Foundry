...
class PipelinePreflightTests(unittest.TestCase):
    ...
    def test_preflight_returns_424_when_tables_are_missing(self) -> None:
        session = MagicMock()
        session.get_bind.return_value = object()
        ...
        self.assertEqual(ctx.exception.error_code, "pipeline_schema_missing")
        self.assertIn("source_documents", ctx.exception.details["missing_tables"])
        self.assertIn("directive_chunks", ctx.exception.details["missing_tables"])
    ...
    def test_preflight_returns_503_when_database_ping_fails(self) -> None:
        ...
        self.assertEqual(ctx.exception.error_code, "pipeline_db_unavailable")
    ...
    def test_preflight_returns_503_when_inspection_fails(self) -> None:
        ...
        self.assertEqual(ctx.exception.status_code, 503)
        self.assertEqual(ctx.exception.error_code, "pipeline_db_inspection_failed")
    ...
    def test_get_calibration_chunks_returns_503_when_query_fails(self) -> None:
        ...
        self.assertEqual(ctx.exception.status_code, 503)
        self.assertEqual(ctx.exception.error_code, "pipeline_db_query_failed")
