...
_REDIS_REPLAY_AUDIT_KEY = "foundry:pipeline:replay_audit"
_DEAD_LETTER_JOBS: list[dict[str, object]] = []
_REPLAY_AUDIT_EVENTS: list[dict[str, object]] = []
    ...
    if client is not None and _queue_backend() == "redis_streams":
        while True:
            res = await client.xreadgroup(
                ...
                count=1,
                block=int(settings.pipeline_stream_block_ms),
            )
            if res:
                _, messages = res[0]
                if messages:
                    ...
                    try:
                        job = _job_from_json(fields.get("data", "{}"))
                    except Exception:
                        await client.xack(_REDIS_STREAM_KEY, _REDIS_STREAM_GROUP, message_id)
                        await _push_dead_letter(
                            PipelineJob(run_id="unknown", cmd=[], env={}, cwd="", attempt=1),
                            f"poison_invalid_payload:{message_id}",
                        )
                        continue
                    return QueueMessage(job=job, ack_id=message_id)
...
async def _stream_reclaim_one(client) -> QueueMessage | None:
    ...
    min_idle_ms = int(settings.pipeline_stream_claim_idle_ms)
    try:
...
async def _record_replay_audit(event: str, run_id: str, details: dict[str, object] | None = None) -> None:
    ...
    client = await _ensure_redis_client()
    if client is not None and _queue_backend() in ("redis", "redis_streams"):
        await client.rpush(_REDIS_REPLAY_AUDIT_KEY, json.dumps(payload, ensure_ascii=False))
        await client.ltrim(_REDIS_REPLAY_AUDIT_KEY, -int(settings.pipeline_replay_audit_retention), -1)
        return
    _REPLAY_AUDIT_EVENTS.append(payload)
    if len(_REPLAY_AUDIT_EVENTS) > int(settings.pipeline_replay_audit_retention):
        del _REPLAY_AUDIT_EVENTS[: len(_REPLAY_AUDIT_EVENTS) - int(settings.pipeline_replay_audit_retention)]
...
@router.get("/deadletter/replay/audit", dependencies=[Depends(require_admin_api_key)])
async def deadletter_replay_audit(limit: int = 50) -> dict[str, object]:
    limit = max(1, min(limit, 500))
    client = await _ensure_redis_client()
    if client is not None and _queue_backend() in ("redis", "redis_streams"):
        rows = await client.lrange(_REDIS_REPLAY_AUDIT_KEY, -limit, -1)
        items: list[dict] = []
        for raw in rows:
            try:
    ...
    return {"count": min(limit, len(_REPLAY_AUDIT_EVENTS)), "items": _REPLAY_AUDIT_EVENTS[-limit:]}
...
async def replay_deadletter(run_id: str) -> dict[str, object]:
    ...
    if job is None:
        await _record_replay_audit("replay_not_found", run_id)
    ...
    if token_estimate > max_budget:
        ...
        await _record_replay_audit(
            "replay_rejected_budget",
            run_id,
            {"estimated_tokens": token_estimate, "max_budget": max_budget},
        )
        raise HTTPException(
            status_code=422,
    ...
    try:
        ...
        await _record_replay_audit("replay_rejected_queue_full", run_id)
        raise ServiceUnavailableError(
            error_code="pipeline_queue_full",
            message="Pipeline queue is full.",
