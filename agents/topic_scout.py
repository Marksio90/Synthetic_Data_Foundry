...
async def _fetch_arxiv(query: str) -> list[ScoutSource]:
    ...
    try:
        ...
        try:
            ...
            logger.warning("feedparser not installed — arXiv crawler disabled")
            return sources
        ...
        for entry in feed.entries[:5]:
            ...
            sources.append(ScoutSource(
                ...
                source_type="arxiv",
                verified=True,  # arxiv.org is always whitelisted and reachable
            ))
    ...
    try:
        ...
        for work in data.get("results", [])[:5]:
            ...
            sources.append(ScoutSource(
                ...
                verified=True,  # URL comes directly from OpenAlex metadata
            ))
    except Exception as exc:
    ...
    try:
        ...
        for hit in data.get("hits", [])[:5]:
            ...
            sources.append(ScoutSource(
                ...
                verified=True,  # HN item URLs are always valid
            ))
    except Exception as exc:
...
async def _fetch_eurlex(query: str) -> list[ScoutSource]:
    ...
    try:
        ...
        for entry in entries_to_check:
            link = entry.get("link", "")
            verify_tasks.append(_verify_url(link))

        verified_results = await asyncio.gather(*verify_tasks, return_exceptions=True)

        for entry, ok in zip(entries_to_check, verified_results):
            if ok is not True:
                continue
            sources.append(ScoutSource(
                url=entry.get("link", ""),
...
async def run_scout(
    ...
    """
    Discover knowledge-gap topics using all available methods in parallel.
    ...
    results: list[ScoutTopicData] = []

    for domain in domains:
        await _log(f"Scanning: {domain[:60]}")

        # Run all crawlers + LLM probe in parallel
        ...
        all_sources: list[ScoutSource] = [
            *arxiv_srcs, *oa_srcs, *hn_srcs, *el_srcs
        ]
        ...
        # Use the freshest source title as the summary
        best = max(
